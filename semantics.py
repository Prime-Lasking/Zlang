...
class SemanticAnalyzer:
    ...
    def _check_variable_exists(self, name: str, line_num: int) -> None:
        ...
        if "[" in name and "]" in name:
            ...
            if not self._get_decl(array_name):
                self._error(
                    f"Array '{array_name}' not declared",
                    line_num,
        ...
        if not decl:
            self._error(
                f"Variable '{name}' not declared", line_num, ErrorCode.UNDEFINED_SYMBOL
            )
    ...
    def _check_type_compatibility(
        self, var_name: str, value_type: str, line_num: int
    ) -> None:
        ...
        # Assignment: LET dest value
            ...
            if is_pointer_deref:
                ...
                if not ptr_decl or not ptr_decl["type"].endswith("*"):
                    self._error(
                        f"Cannot dereference non-pointer variable '{ptr_name}'",
                        line_num,
                        ErrorCode.TYPE_MISMATCH,
                    )
    ...
    def _handle_pointer_operations(
        self, op: str, operands: List[str], line_num: int
    ) -> None:
        ...
        if op == "PTR":
            ...
            if len(operands) != 3:
                self._error(
                    "PTR operation requires 3 operands: type, pointer_name, target_variable",
                    line_num,
                    ErrorCode.SYNTAX_ERROR,
                )
                return
            ...
            if type_name not in types_set:
                self._error(
                    f"Invalid type '{type_name}' for pointer",
                    line_num,
                    ErrorCode.INVALID_TYPE,
                )
                return
    ...
    def _handle_arithmetic(self, op: str, operands: List[str], line_num: int) -> None:
        ...
        if len(operands) != 3:
            self._error(
                f"{op} requires exactly 3 operands", line_num, ErrorCode.SYNTAX_ERROR
            )
    ...
    def _handle_inc_dec(self, op: str, operands: List[str], line_num: int) -> None:
        ...
        if not operands:
            self._error(
                f"{op} requires a variable name", line_num, ErrorCode.SYNTAX_ERROR
            )
    ...
    def _handle_call(self, operands: List[str], line_num: int) -> None:
        ...
        if not operands:
            self._error(
                "Function name expected after CALL", line_num, ErrorCode.SYNTAX_ERROR
            )
            return
        ...
        if "->" in operands:
            ...
            if len(operands) > 1 and operands[-1] != "->":
                ...
                if len(operands) > 2 or (
                    len(operands) == 2 and not operands[1].startswith("(")
                ):
                    return_var = operands[-1]
                    args = operands[
                        1:-1
                    ]  # Everything between function name and return_var
                else:
                    args = operands[
                        1:
        ...
        if (
            ...
            arg_str = args[0][
                1:-1
            ].strip()  # Remove the parentheses and trim whitespace
        ...
        func_decl = self._get_decl(
            func_name, None
        )  # Functions are always in global scope
    ...
    def _handle_ret(self, operands: List[str], line_num: int) -> None:
        ...
        if not self.return_type or self.return_type == "void":
            ...
            if not operands:
                self._error(
                    "Function must return a value", line_num, ErrorCode.TYPE_MISMATCH
                )
            else:
    ...
    def _handle_arr(self, operands: List[str], line_num: int) -> None:
        ...
        if operands[0] not in array_types:
            self._error(
                f"Invalid array type: {operands[0]}", line_num, ErrorCode.INVALID_TYPE
            )
            return
        ...
        if self._get_decl(array_name):
            self._error(
                f"Variable '{array_name}' already declared",
                line_num,
                ErrorCode.DUPLICATE_DECLARATION,
            )
            return
    ...
    def _handle_push_pop(self, op: str, operands: List[str], line_num: int) -> None:
        ...
        if len(operands) < 1:
            self._error(
                f"{op} requires an array name", line_num, ErrorCode.SYNTAX_ERROR
            )
            return
        ...
        if array_type not in array_types:
            self._error(
                f"{op} operation requires an array, got {array_type}",
                line_num,
                ErrorCode.TYPE_MISMATCH,
            )
            return
    ...
    def _handle_len(self, operands: List[str], line_num: int) -> None:
        ...
        if len(operands) != 2:
            self._error(
                "LEN requires an array and a destination variable",
                line_num,
                ErrorCode.SYNTAX_ERROR,
            )
            return
        ...
        if array_type not in array_types:
            self._error(
                f"LEN operation requires an array, got {array_type}",
                line_num,
                ErrorCode.TYPE_MISMATCH,
            )
    ...
    def _handle_print(self, operands: List[str], line_num: int) -> None:
        ...
        if not operands:
            self._error(
                "PRINT requires at least one argument", line_num, ErrorCode.SYNTAX_ERROR
            )
            return
        ...
        for var in operands:
            ...
            # Handle pointer dereferencing (e.g., *ptr)
            elif var.startswith("*") and len(var) > 1:
                ptr_name = var[1:]  # Remove the *
                self._check_variable_exists(ptr_name, line_num)
                # Check if it's actually a pointer
                decl = self._get_decl(ptr_name)
                if not decl or not decl["type"].endswith("*"):
    ...
    def _get_variable_info(
        self, var_name: str, line_num: int
    ) -> Optional[Dict[str, Any]]:
        ...
        if "[" in var_name and "]" in var_name:
            ...
            if not decl.get("is_array", False):
                self._error(
                    f"'{array_name}' is not an array", line_num, ErrorCode.TYPE_MISMATCH
                )
                return None
            return decl
    ...
    def _handle_printarr(self, operands: List[str], line_num: int) -> None:
        ...
        if len(operands) != 1:
            self._error(
                "PRINTARR requires exactly one array variable",
                line_num,
                ErrorCode.SYNTAX_ERROR,
            )
            return
    ...
    def _handle_import(self, operands: List[str], line_num: int) -> None:
        ...
        if len(operands) >= 2:
            self._error(
                "IMPORT cannot have more than one argument",
                line_num,
                ErrorCode.SYNTAX_ERROR,
            )
            return
        ...
        if not os.path.exists(import_path):
            self._error(
                f"Import file not found: {file_name}",
                line_num,
        ...
        try:
            ...
            imported_instructions, imported_variables, imported_declarations = (
                parse_z_file(import_path)
            )

            # Optimize the instructions
            imported_optimized = optimize_instructions(
                imported_instructions, import_path
            )
                    ...
                    if func_name != "main":
                        ...
                        if func_key not in self.declarations:
                            ...
                            if len(operands_list) > 1:
                                ...
                                if (
                                    len(operands_list) % 2 == 2
                                ):  # Even number of items after name -> last is return type
            ...
            self._error(
                f"Failed to import '{file_name}': {str(e)}",
                line_num,
...
for op, handler_name in HANDLERS.items():
    if not hasattr(SemanticAnalyzer, handler_name):
        if handler_name in {
            "_handle_arithmetic",
            "_handle_inc_dec",
            "_handle_pointer_operations",
            "_handle_if_elif_else",
            "_handle_loop_start",
            "_handle_push_pop",
        }:
            ...
            setattr(SemanticAnalyzer, handler_name, make_handler(op, handler_name))
        else:
            ...
            setattr(SemanticAnalyzer, handler_name, make_std_handler(handler_name))


def validate_const_and_types(
    instructions: List[Tuple[str, list, int]],
    declarations: Dict[Tuple[Optional[str], str], Dict[str, Any]],
    z_file: str,
) -> None:
