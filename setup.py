...
def run_setup() -> int:
    ...
    try:
        ...
        if dest_exe.exists():
            msg = (
                f"Current z.exe ({current_resolved}) is already installed at this location." if dest_resolved == current_resolved
                else f"Another z.exe already exists at {dest_exe}."
            )
            print_colored(msg, Colors.YELLOW)
            if not ask_yes_no("Replace the existing z.exe with the one you're running?", default_no=True):
                print_colored("Keeping the existing z.exe and aborting install.", Colors.WHITE)
                return 0
...
def show_setup_message():
    ...
    print()
    print_colored("  z", Colors.GREEN + Colors.BOLD)
    print()
