...
def optimize_instructions(instructions: List[Tuple[str, list, int]]):
        ...
        if op in {"ADD", "SUB", "MUL", "DIV", "MOD"} and len(operands) >= 2:
            ...
            if all(x and (x[0].isdigit() or (x[0] == '-' and len(x) > 1 and x[1].isdigit()) or x[0] == '.') 
                ...
                try:
                    ...
                    elif op == "DIV" and nums[1] != 0: result = nums[0] / nums[1]
                    elif op == "MOD" and nums[1] != 0: result = nums[0] % nums[1]
